...
public class NumberTriangle {
    ...
    public int retrieve(String path) {
        ...
        String next = path.substring(1);
        if (path.charAt(0) == 'l') {
            rtn = left.retrieve(next);
        }
        else if (path.charAt(0) == 'r') {
            rtn = right.retrieve(next);
        }
        return rtn;
    }
    ...
    public static NumberTriangle loadTriangle(String fname) throws IOException {
        ...
        ArrayList<NumberTriangle> past = new ArrayList<NumberTriangle>();

        // will need to return the top of the NumberTriangle,
        // so might want a variable for that.
        String line = br.readLine();
        int root_num = Integer.valueOf(line);

        NumberTriangle top = new NumberTriangle(root_num);
        past.add(top);
        ...
        while (line != null) {
            ...
            for (int i = 0; i < numbers.size(); i++) {
                ...
                if (i == 0){
                    past.get(0).setLeft(this_numtri);
                } else if (i == (numbers.size() - 1)) {
                    past.get(past.size() - 1).setRight(this_numtri);
                }
                else {
                    past.get(i).setLeft(this_numtri); // left of your index
                    past.get(i-1).setRight(this_numtri); // right of your index - 1
                }
